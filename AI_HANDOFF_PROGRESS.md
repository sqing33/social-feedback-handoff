...
- 从账号主页发现并导入作品；
- 笔记归档、收藏、刷新和删除；
- 浏览、点赞、收藏、评论、分享/转发等公开指标展示与账号级统算；
- 公开评论和指标历史展示；
- 按日期、平台、账号和来源进行组织与筛选；
- Sonar 风格统计仪表盘、14 天趋势和高增长内容；
- LiteLLM 单笔记“可能爆点”、账号策略和同赛道热点分析；
- AI 赛道候选标签与人工确认。
...
### 2.4 统计仪表盘与 LiteLLM AI 分析

本轮新增：

- 深色 Sonar 风格分析仪表盘，支持时间、平台、账号和赛道筛选。
- 账号笔记总数及浏览、点赞、收藏、评论、分享/转发总量；互动总量不包含浏览。
- `NULL` 与真实 `0` 严格分离，并返回每项指标的覆盖数量。
- 14 天新增笔记和当前累计互动趋势。
- 高增长内容必须至少两次真实采样，单次采样不进入列表。
- `panel/ai_service.py` 使用 LiteLLM，支持 MiniMax、DeepSeek、OpenAI 兼容服务和本地 Ollama。
- 新笔记导入成功后自动创建单笔记分析任务；重复导入与普通刷新不重复创建任务。
- AI 输出采用固定 JSON 契约，拒绝不存在于输入的 archive_id 证据。
- AI 生成的赛道标签默认为未确认，用户确认后才进入赛道聚合。
- 模型未配置时统计功能保持可用，页面显示“AI 未配置”。
- 当前没有权威风险分类数据，因此页面使用“高增长内容”，不显示虚假“高风险舆情”。
...
- 账号最新指标总量、NULL 语义、分享/转发和 14 天趋势；
- 高增长双采样门槛与平台/账号/赛道筛选；
- LiteLLM 队列去重、证据校验、错误脱敏和未确认赛道隔离；
- Sonar 风格仪表盘桌面与手机布局。
...
panel/social_feedback_backend.py       Flask 面板后端、SQLite、统计和导入边界
panel/ai_service.py                    LiteLLM 分析任务队列与输出校验
...
tests/test_analytics_dashboard.py       统计、趋势、AI 队列和仪表盘测试
...
9. 修改后至少运行 63 项正式模式与分析回归测试、Python 编译检查和前端 JavaScript 语法检查。
10. 对抖音主页问题，应先获得真实页面证据，再做最小解析器改动。
11. 不把高增长称为高风险；当前没有权威风险数据。
12. 不把 AI 生成标签默认视为已确认；账号和赛道聚合只能使用人工确认标签。
13. 不在日志、AI 任务错误或模型输入中写入 Cookie、API Key、临时访问参数和完整评论身份信息。

## 9. 当前结论

项目的主要功能、账号信息视图、统计仪表盘、LiteLLM AI 分析队列、登录资料持久化和正式安全边界已经完成并通过回归。当前没有权威风险分类数据，仪表盘因此只展示可验证的高增长内容。指定的真实抖音账号主页如果仍没有返回可归属的公开作品节点，继续如实返回“未发现作品”。
