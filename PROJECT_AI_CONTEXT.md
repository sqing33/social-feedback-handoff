...
15. 账号统算只能基于每篇笔记最新 `feedback`，不能累计 `metric_history`。
16. 指标缺失保持 `NULL`；统计响应必须返回指标覆盖数量。
17. 高增长内容至少需要两次真实采样；单次采样不能显示为增长。
18. 当前没有权威风险数据，不使用“高风险舆情”标签；只展示可验证的高增长内容。
19. AI 必须使用“可能原因”措辞，证据 archive_id 必须来自本次真实输入。
20. AI 标签默认未确认；只有人工确认后才能进入账号或赛道聚合。
21. AI 输入不得包含 Cookie、API Key、浏览器 Profile、临时 `xsec_token/xsec_source` 或完整分享链接。
22. AI 输出只写 `ai_insights`，不得污染真实归档和指标表。
...
- Sonar 风格分析仪表盘：平台、账号、赛道、日期筛选，笔记/互动总量和 14 天趋势。
- 账号指标统算：每篇笔记只计最新值，严格区分 `NULL` 与真实 `0`。
- 高增长内容：同一作品至少两次真实采样才计算首末互动增量。
- LiteLLM 单笔记、账号和赛道分析队列；新导入笔记自动排队，重复导入和普通刷新不重复收费。
- AI 赛道候选标签与人工确认边界。
- 桌面和手机视口的真实浏览器视觉验收。
