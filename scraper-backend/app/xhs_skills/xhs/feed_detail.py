...
def get_feed_detail(
    ...
    Args:
        ...
        config: 评论加载配置。
        keyword: 搜索词（Level-2 风控用）。
        source: xsec_source 值，必须和 token 颁发场景匹配
            (``pc_search`` / ``pc_feed`` / ``pc_note``)。
            空串时走 :data:`urls.DEFAULT_FEED_SOURCE` (``pc_search``)。
    ...
    url = make_feed_detail_url(feed_id, xsec_token, source=source)
    ...
    for attempt in range(3):
        try:
            page.navigate(url, keyword=(keyword or "").strip())
            page.wait_for_load()
    ...
    # FIX: 帖子被删/不存在时，小红书会 302 重定向到首页或搜索页。
    # 检查当前 URL 是否仍在 explore 页（含 feed_id）。
    try:
        current_url = page.evaluate("window.location.href") or ""
    except Exception:
        current_url = ""
    if f"/explore/{feed_id}" not in current_url and f"/discovery/item/{feed_id}" not in current_url:
        logger.warning(
            "帖子 %s 可能已删除或不可访问（URL 重定向到 %s），跳过",
            feed_id,
            current_url[:120],
        )
        raise PageNotAccessibleError(
            f"帖子不存在或已被删除（被重定向到 {current_url[:80]})"
        )
...
def _load_all_comments(page: Page, config: CommentLoadConfig) -> None:
    ...
    # Wall-clock total timeout (safety net against infinite scroll in feed flow).
    ...
    for attempt in range(max_attempts):
        ...
        # FIX: 评论数从 N 降到 0 → 评论区 DOM 已消失（滚到了推荐流），停止
        if max_count_seen > 0 and current_count == 0:
            logger.warning(
                "评论数从 %d 降到 0，评论区 DOM 可能已消失，停止加载",
                max_count_seen,
            )
            return
        ...
        # FIX: count=0 时不要滚 window（会滚到推荐流），只触发评论区懒加载
        if current_count > 0:
            ...
            # count=0: 评论区还没渲染，用 wheel event 触发懒加载，不要滚 window
            try:
                page.dispatch_wheel_event(100, selector=".comments-container")
            except Exception:
                pass
            sleep_random(*HUMAN_DELAY)
...
def _human_scroll(
    ...
    # BridgePage 将这两个值解析为详情滚动容器的状态；搜索页没有容器时
    # 才回退到 window 页面状态，避免评论加载带动背景搜索流。
    ...
    for i in range(max(1, push_count)):
        ...
        # scroll_by 现在收口到评论容器；容器未就绪时扩展会返回 __xhs_error，
        # 这种情况发生在 tab 刚重建 / 评论区还没渲染的瞬间。此时不要 abort
        # 整个加载流程（外层 count 循环会重试），只把这一步当无操作跳过。
        try:
            page.scroll_by(0, int(scroll_delta))
        except Exception as e:
            logger.debug("scroll_by 容器未就绪，跳过本次: %s", e)
...
def _scroll_to_last_comment(page: Page) -> None:
    ...
    page.evaluate(
        f"""(function(){{
            ...
            if (!el) return;
            const container = el.closest('.note-scroller, .interaction-container');
            if (!container) {{
                return;
            }}
...
def _click_show_more_buttons(page: Page, max_threshold: int) -> tuple[int, int]:
    ...
    for i, text in enumerate(texts):
        ...
        page.evaluate(
            ...
            f"  if(!el) return;"
            f"  var container=el.closest('.note-scroller, .interaction-container');"
            f"  if(!container) return;"
            f"  if(container){{"
            f"    var er=el.getBoundingClientRect();"
            f"    var cr=container.getBoundingClientRect();"
            f"    container.scrollTop += er.top-cr.top-(cr.height-er.height)/2;"
            f"  }}"
            f"  el.click();"
            f"}})()"
        )
